...
class EditableTextClick extends Component{
  constructor(props){
    super(props)
    this.state = {
      name: props.name,
  ...
  render() {
    return (
      ...
      <input
        ...
        onChange={event=>{
          this.setState({value:event.target.value.substring(0, CHAR_LIMIT)})
        }}
        onBlur={event=>{
          this.setState({edit:false});
          this.props.onValueChanged(this.state.value.length?this.state.value.substring(0, CHAR_LIMIT):this.state.backup);
        }}
        onKeyUp={event=>{
          if(event.key==='Escape') {
      ...
      />
      ||
      <div className={this.state.spanClassName} onClick={event=>{
...
export default EditableTextClick;
